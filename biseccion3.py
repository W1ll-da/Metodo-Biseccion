...
def generar_pdf(funcion, xi, xs, xr, iteraciones, ea_deseado, precision):
    ...
    nombre_archivo = f"bisection_{funcion_str.replace('**', '^').replace('*', '').replace('/', '÷').replace(' ', '_')}.pdf"
    ...
    c.drawString(100, y, f"Función: {funcion_str}")
    y -= 20
    ...
    funcion_sustituida_str = re.sub(r'\bx\b', f"({round(xr, precision)})", funcion_str)
    c.drawString(120, y, f"f({round(xr, precision)}) = {funcion_sustituida_str} = {round(f_xr_final, precision)}")
    y -= 40
